...
export async function PunchmoleServer(
  ...
  wss.on("connection", (socket) => {
    log.info(
      ...
      socket._socket.headers,
      socket._socket.origin_url,
    );
    ...
    if (socket._socket.origin_url === endpointUrlPath) {
        ...
        switch (message.type) {
          ...
          case "response-start":
            ...
            if (request) {
              ...
              log.error(
                new Date(),
                "didnt found response object, probably dead?",
              );
            }
            break;
          case "data":
            if (request) {
              ...
              try {
                await request.responseObject.write(data);
              } catch (err) {
              ...
              log.error(
                new Date(),
                "didnt found response object, unable to send data",
                message.id,
              );
            }
            break;
      ...
      const requestedDomain =
        socket._socket.headers.host.match(/^(.*?)(:[0-9]{1,}|)$/)[1];
